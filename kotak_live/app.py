    ...
    def discover_instruments(self) -> bool:
        ...
        for r in records:
            ...
            sym = str(
                r.get(
                    "pTrdSymbol",
                    r.get("tradingSymbol", r.get("ts", r.get("symbol", "")))
                )
            ).upper().strip()
            token = str(
                r.get(
                    "pSymbol",
                    r.get("pSymbolToken", r.get("instrument_token", r.get("token", "")))
                )
            )
            ...
            if any(
                x in sym
                for x in ["BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "SENSEX"]
            ):
                continue
        ...
        if not candidates:
            raise RuntimeError(
                "Active NIFTY future contract could not be discovered from scrip master."
            )
        ...
        self.future_expiry = (
            None if selected_expiry == date.max else selected_expiry
        )
        ...
        for r in records:
            if not isinstance(r, dict):
                continue
            sym = str(
                r.get("pTrdSymbol", r.get("tradingSymbol", ""))
            ).upper().strip()
        ...
        self.log(
            f"Discovery complete: Future={self.future_token}, "
...
class SupabasePublisher:
    ...
    def __init__(self, url_override: str = "", key_override: str = ""):
        self.url = str(
            url_override or env_or_secret(
                "SUPABASE_URL",
                CONFIG["supabase_url"]
            )
        ).strip()
        self.key = str(
            key_override or env_or_secret(
                "SUPABASE_KEY",
                CONFIG["supabase_key"]
            )
        ).strip()
        ...
        for i in range(0, len(raw_payloads), batch_size):
            ...
            try:
                ...
                if response.status_code in (200, 201, 204):
                    ...
                    print(
                        f"Supabase batch publish failed "
...
def main():
    ...
    st.set_page_config(
        page_title="Institutional Raw Data Producer Bus",
        layout="wide"
    )
    ...
    with st.sidebar:
        ...
        if st.button(
            "âœ… Confirm / Apply Configuration",
            type="primary",
        ...
        config_confirmed = bool(
            st.session_state.get("config_confirmed", False)
        )

        if config_confirmed:
            st.caption(
                f"âœ“ Configuration active â€¢ "
        ...
        with c2:
            if st.button(
                "ðŸ”Ž Discover Instruments",
                disabled=not (
                    config_confirmed and kotak.connected
                ),
                use_container_width=True,
            ):
                try:
        ...
        st.caption(
            "yfinance â†’ Supabase only. Requested windows are preserved; "
            "only actual returned source history is published."
        )

        if st.button("Test yfinance Data Source"):
            with st.spinner(
                "Probing yfinance source coverage..."
            ):
        ...
        if st.button(
            "Publish NIFTY History",
            disabled=not (supabase.url and supabase.key)
        ):
            with st.spinner(
                "Publishing NIFTY historical raw data..."
            ):
                count = historical.publish_nifty_history()
            st.success(
                f"NIFTY historical rows published: {count}"
            )

        if st.button(
            "Publish Next-Day 500 History",
            disabled=not (supabase.url and supabase.key)
        ):
            try:
                symbols = [
                    x.strip().upper()
                    for x in hist_symbols_text
                    .replace(",", "\n")
                    .splitlines()
                    if x.strip()
                ]
                ...
                if not symbols:
                    st.warning(
                        "Provide the NIFTY-500 symbol list first."
                    )
                else:
                    with st.spinner(
                        f"Publishing {len(symbols)} symbols Ã— "
                        f"320 daily bars..."
                    ):
                        stats = (
                            historical
                            .publish_next_day_universe_history(
                                symbols
                            )
                        )
                    st.success(
        ...
        if st.button(
            ...
            if not symbols:
                st.warning(
                    "Provide the shortlisted MTF symbols first."
                )
            else:
                with st.spinner(
                    f"Publishing MTF history for {len(symbols)} symbols..."
                ):
    ...
    live_status = (
        "READY"
        if (kotak.connected and kotak.future_token)
        else "NOT READY"
    )
    ...
    if st.session_state.producer_running:
        st.success(
            "ðŸŸ¢ Raw Producer is active. Kotak raw quotes are being "
            "published to Supabase `raw_observations`."
        )
        ...
        while st.session_state.producer_running:
            try:
                ...
                # MACRO RAW DATA
                #
                # Macro raw data is supplementary market context;
                # it remains raw and uncalculated.
                if (
                    poll_cycle
                    % CONFIG["macro_every_n_cycles"]
                    == 0
                ):
                    for ticker, df in (
                        YahooConnector
                ...
                log_container.error(
                    f"Producer loop exception: {exc}"
                )

            time.sleep(
                float(CONFIG["poll_interval_sec"])
            )
