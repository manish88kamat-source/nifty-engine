...
def main():
    ...
    with st.sidebar:
        ...
        if st.button(
            ...
            if not supabase_url_input.strip() or not supabase_key_input.strip():
                st.session_state.config_confirmed = False
                st.error("Enter Supabase URL and Key before confirming.")
            else:
        ...
        if st.button("Test Supabase RAW BUS", disabled=not config_confirmed):
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Connect Kotak", disabled=not config_confirmed):
                try:
                    ...
                    st.error(str(exc))
        with c2:
            if st.button("Discover Instruments", disabled=not (config_confirmed and kotak.connected)):
                try:
    ...
    # Streamlit-native background polling: no blocking while-loop.
    # The fragment reruns independently every poll interval while active.
    @st.fragment(run_every=CONFIG["poll_interval_sec"])
    def live_poll_fragment():
        if not st.session_state.get("producer_running"):
            return
        if not config_confirmed or not kotak.connected or not kotak.future_token:
            st.warning("Producer is marked active but prerequisites are no longer ready.")
            return
        ...
        try:
            ...
            st.error(f"Producer loop exception: {exc}")
