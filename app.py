...
class KotakNeoAdapter:
    def __init__(self):
        ...
        # LIVE instrument identity is resolved from Supabase RAW BUS.
        self.spot_token = ""
        self.spot_symbol = ""
        self.future_token = ""
        self.future_symbol = ""
    ...
    def _contract_info(symbol: str) -> Dict[str, Any]:
        """Parse NIFTY contract identity from the authoritative Supabase symbol."""
        sym = str(symbol or "").strip().upper().replace(" ", "")
        out = {"symbol": sym, "kind": "", "expiry": None, "strike": np.nan, "option_type": ""}
        if not sym:
            return out
        if sym in ("NIFTY50", "NIFTYINDEX"):
            out["kind"] = "SPOT"
            return out
        if sym.startswith("NIFTY") and sym.endswith("FUT"):
            out["kind"] = "FUT"
            m = re.match(r"^NIFTY(\d{2})([A-Z]{3})(?:(\d{2}))?FUT$", sym)
            if m:
                try:
                    ...
                    if m.group(3):
                        ...
                        # Monthly NIFTY future symbol (e.g. NIFTY26SEPFUT):
                        # infer the last Tuesday of that month for expiry metadata.
                        import calendar
                        last_day = calendar.monthrange(year, month)[1]
                        day = last_day
                        while datetime(year, month, day).weekday() != 1:
                            day -= 1
                    out["expiry"] = datetime(year, month, day, tzinfo=IST)
                except Exception:
                    pass
            return out
        if sym.startswith("NIFTY") and (sym.endswith("CE") or sym.endswith("PE")):
            out["kind"] = "OPTION"
            out["option_type"] = sym[-2:]
            # Monthly: NIFTY26SEP24150PE
            # Dated/weekly form (if present): NIFTY26SEP2924150PE
            m_monthly = re.match(r"^NIFTY(\d{2})([A-Z]{3})(\d{5})(CE|PE)$", sym)
            m_weekly = re.match(r"^NIFTY(\d{2})([A-Z]{3})(\d{2})(\d{5})(CE|PE)$", sym)
            m = m_weekly or m_monthly
            if m:
                try:
                    year = 2000 + int(m.group(1))
                    month = datetime.strptime(m.group(2), "%b").month
                    if m_weekly:
                        ...
                        strike_group = 3
                    out["expiry"] = datetime(year, month, day, tzinfo=IST)
                    out["strike"] = float(m.group(strike_group))
                except Exception:
                    pass
            return out
        return out

    def _resolve_authoritative_instruments(self) -> None:
        """Resolve LIVE NIFTY identities from Supabase symbol/token pairs only."""
        with self.lock:
            candidates = []
            for tok, raw in self.latest.items():
                if not isinstance(raw, dict):
                    continue
                sym = str(raw.get("_supabase_symbol") or raw.get("display_symbol") or "").strip()
                if not sym:
                    continue
                info = self._contract_info(sym)
                candidates.append((str(tok), sym.upper(), info))

        # Exact NIFTY 50 index only. CE/PE can never become Spot.
        spot_candidates = [(tok, sym) for tok, sym, info in candidates if info["kind"] == "SPOT"]
        if spot_candidates:
            self.spot_token, self.spot_symbol = spot_candidates[-1]
            self.token_to_symbol[self.spot_token] = "NIFTY_SPOT"

        # Active future = nearest expiry NIFTY...FUT from Supabase.
        today = now_ist().date()
        futures = [(tok, sym, info["expiry"]) for tok, sym, info in candidates
                   if info["kind"] == "FUT" and info["expiry"] is not None]
        valid_futures = [x for x in futures if x[2].date() >= today] or futures
        if valid_futures:
            tok, sym, expiry = sorted(valid_futures, key=lambda x: x[2])[0]
            self.future_token = str(tok)
            self.future_symbol = sym
            self.future_expiry = expiry
            self.token_to_symbol[self.future_token] = "NIFTY_FUT"

        # PCR contracts = real option tokens from Supabase; no dummy token generation.
        option_rows = [(tok, sym, info) for tok, sym, info in candidates
                       if info["kind"] == "OPTION" and info["expiry"] is not None]
        if option_rows:
            expiry_groups = defaultdict(list)
            for item in option_rows:
                expiry_groups[item[2]["expiry"].date()].append(item)
            expiry_dates = [d for d in expiry_groups if d >= today] or list(expiry_groups)
            if expiry_dates:
                chosen_date = min(expiry_dates)
                chosen = expiry_groups[chosen_date]
                self.active_pcr_expiry = next(
                    x[2]["expiry"] for x in chosen if x[2]["expiry"] is not None
                )
                spot_raw = self.latest.get(self.spot_token, {}) if self.spot_token else {}
                spot_px = extract_tick_price(spot_raw)
                if not is_valid_number(spot_px):
                    spot_px = 24300.0
                step = CONFIG["pcr_strike_step"]
                atm = round(float(spot_px) / step) * step
                count = CONFIG["pcr_strike_count"]
                low, high = atm - count * step, atm + count * step
                local = [x for x in chosen if is_valid_number(x[2]["strike"])
                         and low <= float(x[2]["strike"]) <= high]
                self.pcr_tokens = []
                self.pcr_records = {}
                for tok, sym, info in sorted(
                    local, key=lambda x: (float(x[2]["strike"]), x[2]["option_type"])
                ):
                    self.pcr_tokens.append(str(tok))
                    self.pcr_records[str(tok)] = {
        ...
        self.discovery_log.append(
            f"OK Supabase authoritative Spot: {self.spot_symbol or 'NOT FOUND'} | Token: {self.spot_token or 'NOT FOUND'}"
        )
        self.discovery_log.append(
            f"OK Supabase authoritative Future: {self.future_symbol or 'NOT FOUND'} | Token: {self.future_token or 'NOT FOUND'}"
            + (f" | Expiry: {self.future_expiry.date()}" if self.future_expiry else "")
        )
        self.discovery_log.append(
            f"OK Supabase authoritative PCR contracts: {len(self.pcr_tokens)}"
            + (f" | Expiry: {self.active_pcr_expiry.date()}" if self.active_pcr_expiry else "")
        )

    def discover_nifty_instruments(self, auto_pcr: bool = True) -> bool:
        self.discovery_log.clear()
        if not self.connected:
            raise RuntimeError("Connect to Supabase before instrument discovery.")
        ...
        self._resolve_authoritative_instruments()
        return True
    ...
    def fetch_market_snapshot(self):
        if not self.url or not self.key:
            return
        try:
            # NIFTY live consumer reads ONLY Kotak RAW rows from the RAW BUS.
            endpoint = (
                f"{self.url.rstrip('/')}/rest/v1/raw_observations"
                "?select=source,symbol,instrument_token,observation_timestamp,raw"
                "&source=eq.kotak_live&order=id.desc&limit=300"
            )
            headers = {
                "apikey": self.key,
                ...
                if isinstance(rows, list):
                    ...
                    with self.lock:
                        for row in reversed(rows):
                            ...
                            row_tok = row.get("instrument_token")
                            raw_tok = token_from_record(raw)
                            tok = str(row_tok or raw_tok).strip()
                            sym_name = str(
                                row.get("symbol")
                            ...
                            # Missing-token rows are not allowed to masquerade as NIFTY Spot.
                            if not tok:
                                continue

                            raw["_supabase_source"] = str(row.get("source") or "")
                            raw["_supabase_symbol"] = sym_name
                            raw["_supabase_instrument_token"] = tok
                        ...
                        if self.future_token:
                            fraw = self.latest.get(str(self.future_token), {})
                            pdc = safe_float(fraw.get("c") or fraw.get("close") or fraw.get("pdc"))
                            pdh = safe_float(fraw.get("h") or fraw.get("high") or fraw.get("pdh"))
                            pdl = safe_float(fraw.get("l") or fraw.get("low") or fraw.get("pdl"))
                            open_p = safe_float(fraw.get("o") or fraw.get("open"))
                    ...
                    self.last_error = "Supabase returned a non-list response."
            else:
                self.last_error = f"Supabase HTTP {resp.status_code}: {resp.text[:200]}"
    ...
    def _build_option_side(self, option_type, atm, ticks_source, bar_time):
        ...
        spot=extract_tick_price(self.latest.get(str(self.spot_token),{})) if self.spot_token else np.nan;spot=spot if is_valid_number(spot) else float(atm)
    ...
    def _close_bar(self, bar_time: datetime, is_session_end: bool = False):
        with self.lock:
            ...
            spot_key = self.spot_token
            _, spot_prices = _prices(spot_key) if spot_key else ([], [])
            if not spot_prices:
                last = extract_tick_price(self.latest.get(str(spot_key), {})) if spot_key else np.nan
...
def build_human_market_map(feats, v10_state=None):
    ...
    elif d_sweep: location_state = "DEMAND SWEPT â€” RECLAIM PENDING"
    ...
    if d_sweep and d_reclaim and ce > pe and mom >= -0.05:
        if (bos > 0 or choch > 0) and mom >= 0.05:
            setup_state = "CE CONFIRMED â€” STRUCTURE + MOMENTUM"; setup_stage = "CONFIRMED"
        else:
            setup_state = "CE EARLY REVERSAL â€” BUILDING"; setup_stage = "EARLY"
    elif d_sweep and not d_reclaim:
        setup_state = "CE WATCH â€” WAIT FOR DEMAND RECLAIM"; setup_stage = "WAIT_RECLAIM"
    elif is_valid_number(d_low) and price < d_low:
        setup_state = "PE RISK â€” DEMAND INVALIDATED"; setup_stage = "INVALIDATED"
    elif s_sweep and s_reject and pe > ce:
        setup_state = "PE EARLY REVERSAL â€” BUILDING"; setup_stage = "PE_EARLY"
    elif edge >= 8:
        setup_state = "CE BIAS â€” NO CLEAN LOCATION TRIGGER"; setup_stage = "BIAS"
    elif edge <= -8:
        setup_state = "PE BIAS â€” NO CLEAN LOCATION TRIGGER"; setup_stage = "BIAS"
    else:
...
def main():
    ...
    if adapter and adapter.latest:
        with adapter.lock:
            s = adapter.latest.get(str(adapter.spot_token), {}) if adapter.spot_token else {}
            spot_p = extract_tick_price(s)
            spot_val = f"{spot_p:.2f}" if is_valid_number(spot_p) else "-"

            f = adapter.latest.get(str(adapter.future_token), {}) if adapter.future_token else {}
